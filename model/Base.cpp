 ...
 *
 **/
pair<vector<vector<double>>, vector<vector<int>>> Base::get_topk(const vector<vector<string>>& queries, unsigned int n) {
    ...
    for (const auto& score : scores) {
        ...
        sort(indices.begin(), indices.end(), [&score](int i1, int i2) {
    ...
    pair<vector<vector<double>>, vector<vector<int>>> output = { out_scores, out_indices };
    return output;
}
 ...
 *
 **/
vector<vector<string>> Base::get_topk_docs(const vector<vector<string>>& queries, const vector<string>& docs, unsigned int n) {
    vector<vector<double>> scores = get_scores(queries);
